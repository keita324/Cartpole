...
class CollaborativeActor:
    ...
    def get_actor(self):
        """完全にランダムにアクターを選択"""
        return np.random.choice(['mario', 'luigi'])
...
def evaluate_without_learning(env, qn, actor_name, evaluation_type, episode_num):
    ...
    return eval_data  # Fixed: Return eval_data instead of eval
...
def main():
    ...
    mario_videos_dir = os.path.join(base_dir, 'mario_videos')  # マリオの実行動画保存ディレクトリ
    luigi_videos_dir = os.path.join(base_dir, 'luigi_videos')  # ルイージの実行動画保存ディレクトリ
    results_dir = os.path.join(base_dir, 'results')           # 結果保存ディレクトリ

    # 必要なディレクトリの作成
    for directory in [mario_videos_dir, luigi_videos_dir, results_dir]:
    ...
    for trial in range(50):
        ...
        # データ記録用の配列初期化
        steps_history = []      # 各エピソードのステップ数
        actors_history = []     # 各エピソードで選択されたアクター
        evaluation_results = [] # 評価結果
        
        # 学習の終了判定用変数
        total_reward_vec = np.zeros(10)  # 直近10エピソードの報酬を記録
        goal_average_reward = 195        # 目標平均報酬
        islearned = 0                    # 学習完了フラグ
        ...
        while True:
            ...
            if islearned or episode >= NUM_EPISODES:
                ...
                # 学習履歴の保存
                # ステップ数と選択されたアクターの履歴を保存
                if steps_history and actors_history:  # リストが空でない場合のみ保存
